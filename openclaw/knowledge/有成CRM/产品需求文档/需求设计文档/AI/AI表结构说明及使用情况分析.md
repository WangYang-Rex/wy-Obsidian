...
| 可分析维度 | 指标 |
|---------------------------------------------|------------------------------------|
| 基础使用分析 | 1. 会话维度（表1）1. 日均/周均/月均会话量（按企业/用户分组）1. 会话设备分布（PC/Mobile比例）2. 消息维度（表2）2. 日均/周均/月均消息量（按企业/用户分组）3. 消息量级分布（单会话平均消息数）4. 意图类型分布（查询/创建/咨询等数量及占比） |
| 活跃度分析 | 1. 企业活跃度（表1，所在平台/企业规模/行业信息需要联表查有成系列中企业信息表）1. 统计日均会话量\>=3 or 月均会话量\>=60 的企业数（计为活跃企业）2. 统计活跃企业/未到期企业占比3. 统计活跃企业，所在平台、企业规模、行业的分组情况1. 所在平台：架构名 ding74d88e04ad655ab435c2f4657eb6378f ，表名customer ，字段名customer\_select\_152. 企业规模：架构名 ding74d88e04ad655ab435c2f4657eb6378f ，表名customer ，字段名customer\_integer\_0（分组：6人以下、6-50人，50-100，100-500，500-2000，大于2000）3. 行业：架构名 ding74d88e04ad655ab435c2f4657eb6378f ，表名customer ，字段名customer\_input\_44. 统计活跃企业意图类型分组情况2. 用户活跃度（表1，所在平台/企业规模/行业信息需要联表查有成系列中企业信息表）5. 统计日均会话量\>=1 or 月均会话量\>=20 的用户数（计为活跃用户）6. 统计活跃用户/未到期企业用户总数占比7. 统计活跃用户，所在所在平台、企业规模、行业、~~用户角色~~分组情况4. 所在平台、企业规模、行业同企业活跃度5. ~~角色分组：查用户所在企业，角色表（一个用户有多个角色，按角色名称进行分组）~~8. 统计活跃用户意图类型分组情况9. 新用户的首周活跃率（第一次使用的用户，7天内再次使用则算首周活跃，除以AI使用总用户数，得出首周活跃率） |
| 异常使用分析 | 1. ~~无效会话~~~~（表1\+表2）~~1. ~~统计仅包含1条消息的会话数（提示可能为无效会话）~~2. ~~统计无效会话/总会话占比~~2. 重复意图请求（表1\+表2）3. 统计相同意图类型\>=2的会话数（提示可能AI未准确识别）4. 统计重复意图请求会话数/总会话数占比 |
| 跟进记录分析 | 1. 字段填充完整率（AI填充的字段/模板中总字段，得出完整率）（表3）2. 统计单次会话中，新建跟进记录意图\>=2的会话数，及其除以包含新建跟进记录意图会话总数的占比（提示可能AI字段填充可能不精准）（表2） |
```java
