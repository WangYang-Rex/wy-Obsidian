...
- 使用中/使用到期
判断依据：配置/订单中，订单查询列表中
有生效中的订单（生效时间\有订单但是不存在生效中的订单，否则为‘使用到期’
...
点击【配置/订单】可查看服务配置、订单信息
配置
模块/模板
模块：必填，可选线索/客户/联系人/项目/销售机会，变更模块，清除下述所选模板、字段映射配置
模板：非必填，可清空，依据所选模块选择对应模板
其他
当选择的模块为客户
可勾选信息是否同步至公海（默认不勾选，勾选后所属公海必填）
可选择所属公海，非必填
备注：对应处理逻辑以及？中的文案都如下
...
字段映射（参考工商查询字段映射逻辑）
拓客助手字段
企业公开联系方式信息列表：展示为字段名‘可信联系方式’，归为‘手机’类型
企业名称：‘单行文本’类型
企业证件号：‘单行文本’类型
相似企业名：数组中企业名做拼接，如‘企业1,企业2’，‘多行文本’类型
相似原因：‘多行文本’类型
购买力分数：‘整数’类型
购买力得分原因：‘多行文本’类型
机会分析：‘多行文本’类型
...
匹配度得分：‘整数’类型
推荐理由：‘多行文本’类型
主营列表：数组中主营内容做拼接，‘多行文本’类型
标讯公告名称：：‘单行文本’类型
CRM字段
依据所选模块\+所选拓客助手字段所属类型，展示可选字段
默认配置
模块：客户
模板：默认模板
字段映射
...
企业证件号--\>统一信用代码
订单
查询该企业在企信中见创建的订单，按生效时时间倒序排列
支持绑定员工
已过期的订单不支持【绑定员工】操作
选择员工-【确定】时，校验所选人员数须\备注：企业开通试用后，默认可以绑定2个License，给企信下单并获取到License后，其中一个License默认绑定开通人
...
- 信息入库
依据字段映射配置
负责人取值：若入库时选择进入公海/线索池的，负责人为空；否则，取当前操作人
备注：
